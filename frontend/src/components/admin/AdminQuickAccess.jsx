...
export default function AdminQuickAccess({ 
    ...
    return (
        <div className="animate-in fade-in duration-700">
            ...
            {/* Selection Drawer */}
            {selectedTableId && (() => {
                const tableData = getTableStatus(selectedTableId);
                const { status, session, assistance } = tableData;
                if (!session && !assistance && selectedTableId) {
                     // Auto-close if no active reason to stay open (e.g. table cleared)
                     // But we might want to keep it open to show "Available"
                }
                    ...
                    <div className="w-full max-w-lg h-[92vh] bg-[#0c0d0c] border border-white/10 rounded-[3rem] shadow-[0_50px_100px_rgba(0,0,0,0.6)] overflow-hidden flex flex-col">
                        <div className="p-10 border-b border-white/5 flex justify-between items-start">
                            <div className="flex items-center gap-6">
                                <div className="w-16 h-16 rounded-3xl bg-white/5 border border-white/10 flex items-center justify-center">
                                    <span className="text-4xl font-serif font-bold text-white italic">{selectedTableId.replace(/^[TBC]/, '').replace(/^0+/, '')}</span>
                                </div>
                                <div>
                                    <h3 className="text-xs font-black text-nizam-gold uppercase tracking-[0.3em] mb-1">Seating Unit</h3>
                                    <p className="text-2xl font-serif text-white font-bold italic">{status === 'free' ? 'Available' : 'Active Session'}</p>
                                </div>
                            </div>
                            <button onClick={() => setSelectedTableId(null)} className="p-4 bg-white/5 rounded-2xl text-white/20 hover:bg-white/10 hover:text-white transition-all border border-white/10">
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-10 space-y-10 no-scrollbar">
                            {assistance && (
                                <div className="p-8 bg-red-600/10 border border-red-600/30 rounded-[2rem] flex items-center justify-between shadow-2xl">
                                    ...
                                    <button 
                                        onClick={() => {
                                            updateAssistance(assistance.id, 'attended');
                                            // Don't close automatically, let the user see it's resolved
                                        }}
                                ...
                                <div className="p-8 bg-blue-500/5 border border-blue-500/20 rounded-[2rem] space-y-6">
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm font-serif text-white/80 italic">
                                                    {session.prepTime 
                                                        ? `${getRemainingPrepTime(session)} minutes remaining` 
                                                        : 'Set estimated preparation duration'}
                                                </p>
                                            </div>
                                        </div>
                                        {session.prepTime && (
                                            <div className={`text-2xl font-serif font-bold italic ${getRemainingPrepTime(session) < 5 ? 'text-red-500 animate-pulse' : 'text-blue-400'}`}>
                                                {getRemainingPrepTime(session)}M
                                            </div>
                                        )}
                                    </div>
                                    <div className="grid grid-cols-3 gap-3">
                                        {[10, 15, 20].map(mins => (
                                            <button 
                                                key={mins}
                                                onClick={() => handleSetPrepTime(session.id, mins)}
                                                className={`py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${session.prepTime === mins ? 'bg-blue-500 text-white shadow-lg' : 'bg-white/5 text-white/40 border border-white/5 hover:bg-white/10'}`}
                                            >
                                                +{mins} Min
                                            </button>
                                        ))}
                        ...
                        <div className="p-10 bg-white/5 border-t border-white/5 grid grid-cols-2 gap-6">
                            {status === 'billing' ? (
                                ...
                                <button 
                                    onClick={() => {
                                        updateStatus(session.id, 'billed');
                                        setSelectedTableId(null);
                                    }}
                            ...
                            {session && (
                                ...
                                <button 
                                    onClick={() => clearTable(selectedTableId)}
