...
import { FileText, CheckCircle, PoundSterling, CheckSquare, Package, HelpCircle, CalendarDays, LogOut, Utensils, LayoutGrid, ShoppingBag, Columns } from 'lucide-react';
...
export default function AdminSidebar({ activeView, onViewChange, onRefresh, counts = {} }) {
    ...
    return (
        <aside className="w-64 bg-nizam-sidebar hidden md:flex flex-col border-r border-nizam-border pb-6 h-screen overflow-y-auto relative active:outline-none">
            ...
            <div className="px-4 mx-4 py-4 flex items-center gap-3 border-t border-nizam-border/50 bg-nizam-card/30 rounded-xl">
                <div className="w-10 h-10 rounded-full bg-nizam-green flex items-center justify-center text-nizam-gold font-serif text-lg border border-nizam-gold/20 shadow-sm relative overflow-hidden">
                    <div className="absolute inset-0 bg-nizam-gold/10"></div>
                    A
                </div>
                <div>
                    <p className="text-sm font-bold text-nizam-text">System Manager</p>
                    <p className="text-[10px] text-nizam-gold/60 uppercase tracking-widest mt-0.5 font-bold">Connected</p>
                </div>
            </div>
        </aside>
    );
